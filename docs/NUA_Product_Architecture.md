...
Implementation branch: `codex/platform-reliability`  
Status: initial reliability changes implemented for review; broader platform programme remains open. This is not a production-readiness certification.
...
flowchart TD
  ...
  Crew["Crew shell"] --> API
  API --> DB["Main MongoDB"]
  API --> Mirror["Optional in-memory booking mirror"]
...
Evidence: `frontend/src/lib/appShell.js`, `frontend/public/manifest.owner.json`, `frontend/public/manifest.staff.json`, `backend/services/bookings_partner_client.py`, `bookings-api/server.py`, `bookings-api/database.py`, `bookings-api/webhooks.py`.

The webhook worker is added by this branch. The POS-to-Bookings mirror remains in memory: do not confuse its unresolved reliability with the separate Bookings-to-partner webhook recovery improvement.
...
| F04 | POS mirror requires an explicit local business mapping, sends a stable retry key and scopes the remote-ID write by business. HTTP status failures are checked; guest payloads are not printed in error logs. | `backend/services/bookings_partner_client.py`; configuration and tenant regressions |
...
Limitations of F02: it prevents duplicate records for the same retry key, not competing bookings with different keys. Allocation still checks then inserts. Usage logging and webhook enqueue remain separate writes after booking creation; a crash between them can leave missing side effects. Concurrent callers can receive an availability conflict before a winning request is visible and should retry with the same key. A replay returns the current booking state, not an immutable original response.

Limitations of F03: delivery is at least once. Partners must deduplicate event IDs. A crash after the receiver processes an event but before acknowledgement can cause redelivery. Exhausted or unknown-outcome deliveries remain failed for reconciliation. Signing, controlled replay UI, complete outbox atomicity and delivery ordering remain open.
...
| Native booking rules | Rules engine and capacity lock present | Native reservation store | Not performed | Multi-process and real-Mongo contention; lock-expiry correctness |
| Standalone booking boundary | Hardened by F01/F02 | Partner keys and independent DB | Not performed | Allocation atomicity and canonical time handling |
| POS booking mirror | F04 added; still request-spawned | Optional environment mapping | Not performed | Durable atomic enqueue, mapping management, lifecycle reconciliation |
| Bookings webhooks | F03 added | HTTP receiver delivery | Not performed | Resident worker, signing, Mongo recovery/load tests |
...
This configuration and enforcement are a target, not shipped by this branch. Until delivered, do not enable simultaneous sales of the same capacity through both systems.
...
All OPEN items below are unfinished. They must not be reported as delivered because this document exists.
...
| A02 | P0 / OPEN | Booking authority and durable POS outbox | All writers inventoried; atomic enqueue; restart-safe delivery; cancel-before-create and delete/retry tests; owner-visible reconciliation. Depends on authority decision and DB topology |
| A03 | P0 / OPEN | Atomic standalone capacity | Two independent processes competing for last table produce exactly one confirmation. Include reschedule, restore, expired lock, waitlist and cross-midnight cases on real Mongo |
| A04 | P0 / OPEN | Canonical booking times | Store/compare canonical instants; resolve venue timezone; reject invalid ranges and ambiguous DST inputs explicitly. Test equivalent offsets, overnight service and transitions |
| A05 | P0 / OPEN | Booking side effects and webhook security | Booking + usage/event intent atomic; signed timestamped webhooks, receiver replay protection, safe endpoint validation, audited retries, failed-delivery view; kill between each write boundary |
...
1. Deploy the Bookings API changes before enabling the updated POS mirror, so `Idempotency-Key` is actually honoured.
...
3. Run Bookings on a resident service/worker runtime. Request-only serverless execution cannot be assumed to keep the lifespan delivery loop alive. Worker placement is a dependency of staging PR #2, not solved by this branch.
